...
#         val_acc = sess.run(accuracy, feed_dict={x: X_train, y: y_train})
#         if val_acc < max_acc:
#             max_acc = val_acc
#             saver.save(sess, 'ckpt/boston/boston.ckpt', global_step=i + 1)
#             """
#             checkpoint文件是个文本文件，里面记录了保存的最新的checkpoint文件以及其它checkpoint文件列表。
#             .data-00000-of-00001和.index保存了所有的weights、biases、gradients等变量。.meta保存了图结构。
#             """
#             print('Val Error:', val_acc)

"""
加载训练好的模型:
"""
with tf.compat.v1.Session() as sess:
    # tf.train.import_meta_graph加载保存好的图结构（.meta）
    new_saver = tf.train.import_meta_graph('ckpt/boston/boston.ckpt-995.meta')
    # tf.train.latest_checkpoint会自动找到最近保存的变量文件
    new_saver.restore(sess, tf.train.latest_checkpoint('ckpt/boston'))
    accuracy = tf.reduce_mean(tf.cast(correct_prediction, 'float'))
    val_acc = sess.run(accuracy, feed_dict={x: X_train, y: y_train})
    print('model_Val:', val_acc)
    ...
    # # 输出某一层网络权重
    # import tflearn
    # fc1_vars = tflearn.variables.get_layer_variables_by_name('fc1')
    # # Get a variable's value, using model `get_weights` method:
    # print("FC1 layer weights:")
    # print(fc1_vars)
